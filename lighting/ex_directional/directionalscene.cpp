...
DirectionalScene::DirectionalScene( const QString& fileName )
    : m_fileName( fileName ),
...
void DirectionalScene::initialise()
{
    // Load the model mesh
    ObjLoader loader;
    ...
    material->setShaders( ":/shaders/directional.vert",
...
void DirectionalScene::update( float t )
{
    Q_UNUSED( t );
...
void DirectionalScene::render()
{
    ...
    {
        case Directional:
            shader->setUniformValue( "light.position", QVector4D( -2.0f, 0.0f, 1.0f, 0.0f ) );
            break;
        case Point:
            shader->setUniformValue( "light.position", QVector4D( -2.0f, 0.0f, 1.0f, 1.0f ) );
            break;
    }
...
void DirectionalScene::resize( int w, int h )
{
