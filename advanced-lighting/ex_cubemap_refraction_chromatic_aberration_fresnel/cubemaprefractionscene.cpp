...
void CubeMapRefractionScene::render()
{
    ...
    QVector3D eta( 0.955f, 0.96f, 0.965f );

    // f approximates the minimum reflection factor
    float f = ( ( 1.0f - eta.y() ) * ( 1.0f - eta.y() ) )
            / ( ( 1.0f + eta.y() ) * ( 1.0f + eta.y() ) );

    shader->setUniformValue( "drawSkyBox", false );
    shader->setUniformValue( "f", f );
    shader->setUniformValue( "fresnelPower", 5.0f );
    shader->setUniformValue( "material.eta", eta );
    shader->setUniformValue( "material.reflectionFactor", 0.2f );
