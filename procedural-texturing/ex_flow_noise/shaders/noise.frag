...
void main()
{
    ...
    // Slow down animation time a little
    float t = 0.1 * time;

    // Generate 3 octaves of noise
    float n1 = srdnoise( texCoord * 0.5, 0.2 * t, g1 );
    float n2 = srdnoise( texCoord * 2.0 + g1 * 0.5, 0.51 * t, g2 );
    float n3 = srdnoise( texCoord * 4.0 + g1 * 0.5 + g2 * 0.25, 0.77 * t, g2 );

    // Sum them with decreasing weights
    float totalNoise = n1 + 0.75 * n2 + 0.5 * n3;

    // Generate final fragment color
    fragColor = vec4( vec3( 0.4, 0.5, 0.6 ) + vec3( totalNoise ), 1.0 );
}
