...
void main()
{
    ...
    vec4 texture0Color = texture( texture0, texCoord );
    vec4 texture1Color = texture( texture1, texCoord );

    // Blend the two texture colors together using the alpha of the second texture
    // and the factor uniform as the blending factor
    vec4 textureColor = mix( texture0Color, texture1Color, factor * texture1Color.a );
