...
{
    float intensity = abs(sin(texCoord.s * M_PI * steps)) * abs(sin(texCoord.t * M_PI * steps));
    return mix(vec3(1.0, 1.0, 1.0), vec3(1.0, 0.0, 0.0), intensity);
}
...
vec3 proceduralNormal(vec2 texCoord, float steps)
{
    ...
    vec2 p = fract( vec2( 44.0 * texCoord.s, 22.0 * texCoord.t ) ) - vec2( 0.5 );
...
void main()
{
    ...
    vec3 diffuseColor = vec3( 0.0, 0.7, 1.0 );

    vec3 normal = proceduralNormal( texCoord, 50 );
